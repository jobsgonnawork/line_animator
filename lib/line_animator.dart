...
class PointInterpolator {
  ...
  void buildPointsMap() {
    ...
    for (var c = 0; c < points.length - 1; c++) {
      ...
      pointDistanceSteps.add(PercentageStep(distance: totalDistance, percent: 0.0));
  ...
  InterpolatedResult interpolate(controllerValue, animValue, interpolateBetweenPoints) {
    ...
    for (var c = lastPointIndex; c < points.length; c++) {
      if (animValue >= pointDistanceSteps[c].distance) {
        ...
        if (interpolateBetweenPoints) {
          ...
          interpolatedPoint = LatLng(intermediateLat, intermediateLon); // last tail point
    ...
    if (thisPoint == null) {
      thisPoint = LatLng(points[lastPointIndex - 1].latitude, points[lastPointIndex - 1].longitude);
    }

    double angle = 0.0;
    if (_previousPoint != null) if (thisPoint != null) {
      angle = -atan2(thisPoint.latitude - _previousPoint?.latitude, thisPoint.longitude - _previousPoint?.longitude) - 4.7128;
    }
    ...
    return InterpolatedResult(point: thisPoint, angle: _lastAngle, animValue: animValue, controllerValue: controllerValue, builtPoints: builtPoints);
  }
...
class _LineAnimatorState extends State<LineAnimator> with TickerProviderStateMixin {
  ...
  void startAnimation() {
    interpolator = PointInterpolator(originalPoints: widget.originalPoints, distanceFunc: null, isReversed: widget.isReversed);

    if (controller == null) controller = AnimationController(duration: widget.duration, vsync: this);

    animation = Tween<double>(begin: widget.begin, end: interpolator.totalDistance).animate(controller)
      ..addListener(() {
        InterpolatedResult interpolatedResult = interpolator.interpolate(controller.value, animation.value, widget.interpolateBetweenPoints);
  ...
  void didUpdateWidget(LineAnimator oldWidget) {
    if ((oldWidget.begin != widget.begin) || (oldWidget.originalPoints != widget.originalPoints || (oldWidget.isReversed != widget.isReversed))) {
